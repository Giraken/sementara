...
class Customer extends Model
{
    ...
    public const ATTACHMENTS_DIR = 'home/attachments'; // storage/customers/000000

    public const TEMPLATES_DIR = 'home/templates';  // storage/customers/000000/home/files

    public const PRODUCT_DIR = 'home/products';  // storage/customers/000000/home/files
     ...
     */
    protected $fillable = [
        'id', 'uuid', 'name', 'timezone', 'language_id', 'color_scheme', 'text_direction', 'menu_layout', 'theme_mode',
    ];
    ...
    public static function select2($request)
    {
        ...
        $query = Customer::getAll()->leftJoin('users', 'users.customer_id', '=', 'customers.id');
        if (isset($request->q)) {
            $keyword = $request->q;
            $query = $query->where(function ($q) use ($keyword) {
                $q->orwhere('users.first_name', 'like', '%' . $keyword . '%')
