...
class Plan extends Model
{
    ...
    public static function select2($request)
    {
        ...
        // Read all check
        if ($request->user()->admin && !$request->user()->admin->can('readAll', new Plan())) {
            $query = $query->where('plans.admin_id', '=', $request->user()->admin->id);
        }

        if ($request->change_from_uid) {
            $plan = Plan::findByUid($request->change_from_uid);
    ...
    public static function whereAvailablePlans($query = null)
    {
        return $query
    ...
    {
        $plan = new self();
        $plan->currency_id = Currency::first()->id;
    ...
    public function rules()
    {
        ...
        if ($this->getOption('sending_server_option') == Plan::SENDING_SERVER_OPTION_SUBACCOUNT) {
    ...
    public static function defaultOptions()
    {
        ...
        foreach (SendingServer::types() as $key => $type) {
    ...
    {
        return $this
            ->belongsToMany(Currency::class, 'plan_prices')
            ->using(PlanPrice::class)
            ->withPivot(['id', 'price', 'price_per_set']);
    }
    ...
    public function displayTotalQuota()
    {
        if ($this->getOption('email_max') == -1) {
            ...
            return Tool::format_number($this->getOption('email_max'));
        }
    }
    ...
    {
        if ($this->getOption('sending_server_option') != Plan::SENDING_SERVER_OPTION_OWN) {
    ...
    {
        return $this->getOption('sending_server_option') == Plan::SENDING_SERVER_OPTION_SYSTEM;
    }
    ...
    public function updateFitnesses($hash)
    {
        foreach ($hash as $uid => $value) {
            ...
            PlansSendingServer::where('sending_server_id', '=', $sendingServer->id)
    ...
    public function getEmailVerificationServers()
    {
        if ($this->getOption('all_email_verification_servers') == 'yes') {
            $result = EmailVerificationServer::getAllAdminActive()->get()->map(function ($server) {
                return $server;
            });
        } else {
    ...
    {
        return $this->getOption('sending_server_option') == Plan::SENDING_SERVER_OPTION_OWN;
    }
